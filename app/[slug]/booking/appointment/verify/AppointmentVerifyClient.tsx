...
export function AppointmentVerifyClient({ dict }: { dict: unknown }) {
      ...
      <div className="w-full max-w-sm text-center">
        {status === 'loading' && (
          <>
            ...
            <h1 className="text-xl font-bold mb-2 text-[var(--ink)]">{t(dict, 'booking.appointment.verifying')}</h1>
            <p className="text-sm text-[var(--ink-soft)]">{t(dict, 'booking.appointment.verifyingDesc')}</p>
          </>
        )}
        {status === 'success' && (
          <>
            ...
            <h1 className="text-2xl font-bold mb-2 text-[var(--ink)]">{t(dict, 'booking.appointment.verified')}</h1>
            <p className="text-sm mb-8 text-[var(--ink-soft)]">{t(dict, 'booking.appointment.verifiedDesc')}</p>
        ...
        {status === 'alreadyVerified' && (
          <>
            ...
            <h1 className="text-2xl font-bold mb-2 text-[var(--ink)]">{t(dict, 'booking.appointment.alreadyVerified')}</h1>
            <p className="text-sm mb-8 text-[var(--ink-soft)]">{t(dict, 'booking.appointment.alreadyVerifiedDesc')}</p>
            <a href={`/${slug}`} className="inline-block px-6 py-3 rounded-xl font-semibold text-sm bg-[var(--accent)] text-[var(--ink)] pressable shadow-[var(--shadow-accent)] no-underline">
              {t(dict, 'booking.appointment.backToHome')}
            </a>
          </>
        ...
        {status === 'error' && (
          <>
            ...
            <h1 className="text-2xl font-bold mb-2 text-[var(--ink)]">{t(dict, 'booking.appointment.invalid')}</h1>
            <p className="text-sm mb-8 text-[var(--ink-soft)]">{t(dict, 'booking.appointment.invalidDesc')}</p>
            <a href={`/${slug}`} className="inline-block px-6 py-3 rounded-xl font-semibold text-sm bg-[var(--accent)] text-[var(--ink)] pressable shadow-[var(--shadow-accent)] no-underline">
              {t(dict, 'booking.appointment.backToHome')}
            </a>
          </>
